...
struct CalendarTabView: View {
    ...
    @State private var includePaidExpenses = false
    ...
    private var remainingBudget: Double {
        ...
        return budget.semesterBudget - totalPlanned - selectedWishlistTotal
    }
    ...
    private var weeklyPlannerCard: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            // Toggle for including paid expenses
            Toggle(isOn: $includePaidExpenses) {
                HStack(spacing: 8) {
                    Image(systemName: "dollarsign.circle.fill")
                        .foregroundStyle(.green)
                        .font(.caption)
                    Text("Include paid expenses")
                        .font(.subheadline)
                }
            }
            .tint(.green)
            ...
            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 8) {
                ForEach(0..<7, id: \.self) { dayIndex in
                    ...
                    let plannedDayTotal = currentWeeklyPlan?.totalForDay(dayIndex) ?? 0
                    let expenseDayTotal = expensesForDay(dayIndex)
                    let dayTotal = plannedDayTotal + (includePaidExpenses ? expenseDayTotal : 0)
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        if events.isEmpty && (!includePaidExpenses || expenseDayTotal == 0) {
                            Text("No events")
                                .font(.caption2)
                            ...
                            // Paid expenses (when toggle is on)
                            if includePaidExpenses && expenseDayTotal > 0 {
                                HStack {
                                    VStack(alignment: .leading, spacing: 2) {
                                        Text("Paid Expenses")
                                            .font(.caption)
                                            .lineLimit(1)
                                        Text("$\(expenseDayTotal, specifier: "%.0f")")
                                            .font(.caption2)
                                            .foregroundStyle(.red)
                                    }
                                    Spacer()
            ...
            let expensesTotal = expensesForWeek()
            let totalWithExpenses = plannedTotal + (includePaidExpenses ? expensesTotal : 0)
            let difference = weeklyBudget - totalWithExpenses
            
            HStack(spacing: 12) {
                VStack(alignment: .leading, spacing: 4) {
                    Text(includePaidExpenses ? "Total" : "Planned")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                    Text("$\(totalWithExpenses, specifier: "%.2f")")
                        .font(.subheadline)
                        .fontWeight(.bold)
                        .foregroundStyle(.yellow)
                    if includePaidExpenses && expensesTotal > 0 {
                        Text("($\(plannedTotal, specifier: "%.0f") planned + $\(expensesTotal, specifier: "%.0f") paid)")
                            .font(.caption2)
                            .foregroundStyle(.secondary)
                    }
                }
    ...
    // Calculate expenses for a specific day of the week (0 = Monday, 6 = Sunday)
    private func expensesForDay(_ dayIndex: Int) -> Double {
        let calendar = Calendar.current
        ...
        guard let dayDate = calendar.date(byAdding: .day, value: dayIndex, to: selectedWeekStart) else {
            return 0
        }
    ...
    private func expensesForWeek() -> Double {
        let calendar = Calendar.current
        let weekEnd = calendar.date(byAdding: .day, value: 6, to: selectedWeekStart) ?? selectedWeekStart
        
        let weekStartDay = calendar.startOfDay(for: selectedWeekStart)
        let weekEndDay = calendar.date(byAdding: .day, value: 1, to: calendar.startOfDay(for: weekEnd)) ?? weekEnd
        
        // Sum expenses for the week
        return expenses.filter { expense in
            let expenseDate = calendar.startOfDay(for: expense.date)
            return expenseDate >= weekStartDay && expenseDate < weekEndDay
        }.reduce(0) { $0 + $1.amount }
    }
}
