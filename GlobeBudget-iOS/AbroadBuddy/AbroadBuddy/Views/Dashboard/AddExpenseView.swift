...
struct AddExpenseView: View {
    ...
    @State private var showTripExpensePrompt = false
    @State private var currentOngoingTrip: Trip?
    @State private var isPlannedExpense: Bool = false
    ...
    // Check if expense date falls within an ongoing trip
    private var ongoingTripForExpenseDate: Trip? {
        ...
        return trips.first { trip in
            ...
            return expenseDate >= tripStart && expenseDate <= tripEnd && trip.plannedCost > 0
        }
    }
        ...
        NavigationStack {
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    if selectedTab == .expense {
                        Button("Add") {
                            // Check if expense is during an ongoing trip
                            if let trip = ongoingTripForExpenseDate {
                                currentOngoingTrip = trip
                                showTripExpensePrompt = true
                            } else if budget?.mode == .remaining {
            ...
            .alert("Confirm Expense", isPresented: $showExpenseConfirmation) {
                Button("Cancel", role: .cancel) {
                    currentOngoingTrip = nil
                    isPlannedExpense = false
                }
                Button("Add Expense") {
                    if let trip = currentOngoingTrip, isPlannedExpense {
                        addExpenseAsPlanned(trip: trip)
                    } else {
                        addExpense()
                    }
                }
                ...
                if let budget = budget, expenseAmountUSD > 0 {
                    if let trip = currentOngoingTrip, isPlannedExpense {
                        let transferAmount = min(expenseAmountUSD, trip.plannedCost)
                        Text("$\(String(format: "%.2f", transferAmount)) will be transferred from planned to recently paid. The full expense ($\(String(format: "%.2f", expenseAmountUSD))) will be subtracted from your remaining balance. Your new remaining balance will be $\(String(format: "%.2f", budget.semesterBudget - expenseAmountUSD))")
                    } else {
            ...
            .alert("Trip Expense", isPresented: $showTripExpensePrompt, presenting: currentOngoingTrip) { trip in
                Button("Part of Planned Expense") {
                ...
                Text("You're adding an expense during \"\(trip.name)\". Is this part of your planned expense ($\(trip.plannedCost, specifier: "%.2f")) or an additional expense?")
            }
        }
        .task {
    ...
    private func addExpenseAsPlanned(trip: Trip) {
        let finalAmount = expenseAmountUSD
        guard finalAmount > 0 else { return }
        ...
        // Add note that this was part of planned expense
        let plannedNote = "Part of planned expense for \(trip.name)"
        expenseNotes = expenseNotes.isEmpty ? plannedNote : "\(expenseNotes)\n\(plannedNote)"
        ...
        // Transfer from plannedCost to recentlyPaidCost
        // If expense is more than planned, transfer all planned and keep rest as additional
        let transferAmount = min(finalAmount, trip.plannedCost)
        trip.plannedCost -= transferAmount
        trip.recentlyPaid = (trip.recentlyPaid) + transferAmount
        trip.updatedAt = Date()
        
        // For remaining budget mode, subtract the FULL expense amount from budget
        // This reduces both the remaining budget AND the planned amount
        if let budget = budget, budget.mode == .remaining {
            budget.semesterBudget -= finalAmount
            budget.updatedAt = Date()
        }
...
#Preview {
    AddExpenseView()
        .modelContainer(for: [Budget.self, Expense.self, Trip.self], inMemory: true)
}
